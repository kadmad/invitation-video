...
export default function AEImportModal({ templateId, sortOrderStart, fallbackFontId, onClose, onImported }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-surface rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-edge">
          <div>
            <h2 className="text-base font-semibold text-ink">Import from After Effects</h2>
            <p className="text-xs text-ink-muted mt-0.5">
              Font, color, position, and start/stop time only — animation stays manual.
            </p>
          </div>
          <button onClick={onClose} className="text-ink-muted hover:text-ink-muted transition-colors">
          ...
          {!preview ? (
            <div>
              ...
              <button
                ...
                disabled={loading}
                className="w-full border-2 border-dashed border-edge rounded-xl py-10 flex flex-col items-center gap-2 hover:border-primary-300 hover:bg-primary-50/30 transition-colors disabled:opacity-50"
              >
              ...
              <p className="text-[11px] text-ink-muted mt-3">
                ...
                <code className="bg-surface-alt px-1 py-0.5 rounded">scripts/ae-export/export_text_layers.jsx</code>
              </p>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <p className="text-sm text-ink-muted">
                  ...
                  <span className="text-ink-muted"> · {preview.layers.length} text layer(s) · {matchedCount} font(s) matched</span>
                </p>
                <button
                      ...
                      <tr key={i} className="border-t border-edge">
                        ...
                        <td className="px-3 py-2 text-ink">
                          ...
                          {row.text && row.text !== row.name && (
                            <span className="block text-[10px] text-ink-muted mt-0.5">layer: {row.name}</span>
                          )}
                        </td>
                        <td className="px-3 py-2">
                          {row.matched_font_id ? (
                            <span className="text-ink-muted">{row.matched_font_name}</span>
                          ) : (
                        ...
                        <td className="px-3 py-2">
                          {row.color ? (
                            <span className="inline-flex items-center gap-1.5 text-ink-muted text-xs">
                              <span
                                className="w-3.5 h-3.5 rounded border border-edge inline-block"
        ...
        {preview && (
          <div className="px-5 py-4 border-t border-edge flex justify-end gap-2">
